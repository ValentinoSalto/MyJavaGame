...
public class Boss1 {
	...
	public Boss1(float x, float y, float ancho, float alto) {
		...
		idleTexture = new Texture(Gdx.files.internal("Personajes/Boss1/idle.png"));
		Texture walkingLeftTexture = new Texture(Gdx.files.internal("Personajes/Boss1/walk.png"));
		// Texture walkingLeftTexture = new
		// Texture(Gdx.files.internal("Personajes/Knight_2/Walk_left.png"));

		// Divide las texturas en regiones para las animaciones
		TextureRegion[][] idleFrames = TextureRegion.split(idleTexture, idleTexture.getWidth() / 8,
		...
		for (int i = 0; i < 8; i++) {
			...
			idleAnimation = new Animation<>(1 / 10f, idleFrames[0]);
			time = 0f;
		}

		TextureRegion[][] walkingLeftFrames = TextureRegion.split( walkingLeftTexture,
				 walkingLeftTexture.getWidth() / 8,  walkingLeftTexture.getHeight());
		...
		// attackTexture.getWidth()/8,attackTexture.getHeight());
		 ...
		 * walkingRightAnimation = new Animation<>(1 / 6f,
		 * regionsMovement_walking_right);
		 */

		// Crea las animaciones con las regiones correspondientes

		/*
		 * //Animacion WALKING LEFT for (int i = 0; i < 8; i++) {
		 * regionsMovement_walking_left[i] = walkingLeftFrames[0][i];
		 * walkingLeftAnimation = new Animation<>(1 / 6f, walkingLeftFrames[0]); time =
		 * 0f; } //Animacion WALKING RIGHT for (int i = 0; i < 8; i++) {
		 * regionsMovement_walking_right[i] = walkingRightFrames[0][i];
		 * walkingRightAnimation = new Animation<>(1 / 6f, walkingRightFrames[0]); time
		 * = 0f; }
		 */
